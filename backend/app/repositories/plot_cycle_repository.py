...
async def _resolve_lot_fields(
    ...
    *,
    ...
    """Generate the Auto Lot for a NEW cycle (round 8-5A; formula V2 round
    8-12A). Returns (lot_no, lot_no_source, lot_running_no,
    auto_lot_series_key):

      - cycleLabel + supplierCode + pCode all present → AUTO: next running
        number in the (supplier, cycleLabel, pCode) series + generated
        {cycleLabel}-{supplierCode}-{pCode}-{running} (may raise
        LotNumberTooLongError).
      - any missing component → ALWAYS raise AutoLotMissingComponentError
        naming the blank field(s). A missing component is a data problem the
        user must fix, so the transaction rolls back rather than producing an
        ACTIVE cycle with no lot identifier at all (round 8-12A.1).

    Round A — this is now the ONLY way a lot is ever written. The Manual
    branch (a client-supplied lot_no winning over Auto) is GONE, together with
    the `lot_no` parameter that selected it: an Auto Lot is decided once, when
    the cycle is created, and no later edit can change or regenerate it. Rows
    already stored as 'manual' or 'legacy' are untouched and still read
    normally — this binds only what is being WRITTEN now.
...
async def create_cycle(
    ...
    *,
    ...
    ALWAYS generated by _resolve_lot_fields:
    {cycleLabel}-{supplierCode}-{pCode}-{running}, raising
    AutoLotMissingComponentError when a component is blank. This is the shared
    create path for the start-cycle / plot-with-cycle / rollover endpoints AND
    the Excel import.

    Round A — there is no caller-supplied lot any more: every new cycle gets an
    Auto Lot, and nothing can overwrite it afterwards (update_cycle never
    touches the lot columns).

    supplier_lot_no is stored verbatim beside the system lot and never
    influences the generated lot or its running number.
    ...
    resolved_lot, lot_source, running, series_key = await _resolve_lot_fields(
        db, plot, cycle_label=label, p_code=code,
    )
    cycle = PlotCycle(
        plot_id=plot.id,
...
async def update_cycle(
    db: AsyncSession, cycle: PlotCycle, fields: dict
) -> PlotCycle:
    ...
    PRESENCE means "the client sent this":
      - po_number / p_code / supplier_lot_no present → normalized and set (PO
        upper-cased). All three are independent of the system lot; in
        particular supplier_lot_no NEVER regenerates it.

    Round A — an edit NEVER touches the lot columns (lot_no / lot_no_source /
    lot_running_no / auto_lot_series_key). A cycle's Auto Lot is decided once,
    when the cycle is created, and is immutable from then on: renaming a cycle
    or correcting its P.Code no longer renumbers, regenerates or rewrites the
    lot identifier that may already be printed on shipped goods, and there is
    no longer any way to replace it with a hand-typed one. A legacy cycle
    stored with no lot at all stays that way — filling it in would be another
    write path, which is exactly what this round removes. `lot_no` is
    therefore ignored even if a caller puts it in `fields`.
