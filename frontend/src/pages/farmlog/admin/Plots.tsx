...
import { autoPlotCodePreview } from '../../../lib/plot-code';
...
const createPlotSchema = z.object({
  ...
  // Round B — OPTIONAL. Blank means "let the server generate it"
  // ({supplierCode}-{YYMM}-{running}); a typed value is still honoured
  // verbatim, which is why the max length stays.
  plotCode: z.string().max(50).optional().or(z.literal('')),
  name: z.string().min(1, 'กรุณาระบุชื่อแปลง').max(255),
  ...physicalPlotFields,
  ...cyclePlanFields,
...
function CreatePlotModal({
  ...
  // Round B — what the server will mint if the รหัสแปลง box is left blank.
  // Recomputed from the CURRENT supplier + planting date so the user sees the
  // real month before saving (a plot registered in April for a May planting
  // previews 2605). The "###" is honest: the running number is allocated
  // server-side at save, under that supplier's month series.
  const plotCodePreview = autoPlotCodePreview(selectedSupplierCode, watch('plantingDate'));
  ...
  async function onSubmit(values: CreatePlotFormValues) {
    ...
    const payload: PlotWithCycleCreatePayload = {
      plot: {
        ...
        // Round B — null (not '') when the box is empty: that is what asks the
        // backend to generate the code. An empty string would be a "supplied"
        // code and fail validation.
        plotCode: values.plotCode?.trim() || null,
        name: values.name,
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 overflow-y-auto px-6 py-5">
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {/* Round B — the code is generated by default; the box stays
                  for the deliberate case of mirroring a code that already
                  exists on paper or in another system. */}
              <Field label="รหัสแปลง" error={errors.plotCode?.message}>
                <input
                  {...register('plotCode')}
                  className="field-input uppercase"
                  placeholder={plotCodePreview}
                />
                <p className="text-xs text-muted-foreground">
                  เว้นว่างเพื่อให้ระบบสร้างให้ — {plotCodePreview}
                </p>
              </Field>
